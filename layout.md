...
```
Typst 原生支持 CJK、自动书签、`#outline()` 一遍出 TOC。脚本从本文件读取配色、字号、边距。
...
Phase 5 排版前，扫描手稿文件（如 `manuscript/ch01.md`）的 Markdown 源码，检查是否存在以下明显格式问题。存在任意一项时，建议用户先格式化手稿：
