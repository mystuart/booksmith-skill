...
> 上表中所有间距值单位为 em。H1/H2/H3 列表示该级标题的上方/下方间距。
...
# 覆盖页边距（单位 mm）
margins: { top: 25, right: 22, bottom: 28, left: 22 }

# 覆盖正文字号（单位 pt）
body_size: 10.5

# 覆盖 H1 标题上方/下方间距（单位 em）
h1_above: 2.5
h1_below: 1.5

# 覆盖 H2 标题上方/下方间距（单位 em）
h2_above: 1.8
h2_below: 1.2

# 覆盖 H3 标题上方/下方间距（单位 em）
h3_above: 1.5
h3_below: 0.8
...
Python _STYLE_PRESETS（硬编码默认值，必须匹配本文件风格参数总览表）
    ↓  被覆盖
项目 layout.md 参数覆盖（用户在书籍项目的 layout.md 中用 YAML 语法覆盖）
    ↓  被覆盖
...
排版脚本 `booksmith-typst.py` 解析此处的 YAML 代码块作为参数源，不再依赖自然语言正则。此 YAML 块是排版参数的唯一真相源。
...
在书籍项目的 `layout.md` 中，只需复制上面的 `booksmith_layout:` 代码块，修改你想覆盖的参数即可。未指定的参数仍使用上表中的默认值。

```yaml
booksmith_layout:
  layout_style: modern
  leading: 1.85         # 只需改这一项，其他不变
  h3_below: 0.7
```
...
```bash
# 默认风格（从 layout.md 的 booksmith_layout YAML 块读取）
...
- 解析 `layout.md` 的 `booksmith_layout` YAML 代码块提取参数
